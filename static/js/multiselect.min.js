if (!m_helper)
{
    var m_helper = {
        ...
        insertAfter: function (item, target)
        {
            ...
            if (target.nextElementSibling) { parent.insertBefore(item, target.nextElementSibling) }
        ...
        create: function (data) {
            ...
            if (data.attributes)
            {
                for (var prop in data.attributes) { result.setAttribute(prop, data.attributes[prop]) }
            }
        ...
        textField: function (data) {
            ...
            data.tag = 'input';
            if (!data.attributes)
                data.attributes = new Object(); data.attributes.type = 'text'; return this.create(data)
        },
        ...
        click: function (element)
        {
            if (element.fireEvent) { el.fireEvent('onclick') }
                ...
                evObj.initEvent('click', !0, !1); element.dispatchEvent(evObj)
            }
        },
...
Multiselect.prototype =
{
    _createUI: function () {
        ...
        var wrapper = this._createWrapper(); m_helper.insertAfter(wrapper, this._item); wrapper.appendChild(this._createInputField()); wrapper.appendChild(this._createItemList()); m_helper.hide(this._item)
    },
    ...
    _createInputField: function () {
        var input = m_helper.textField({
            ...
            label = m_helper.label({
                ...
                class: 'multiselect-count', attributes: { for: this._getInputFieldIdentifier() }
            }),
    ...
    _initSelectedFields: function () {
        var itemResult = this._getItems().filter(function (obj) { return obj.selected });
    ...
    _appendEvents: function () {
        ...
        { self._showList(self); document.getElementById(self._getInputFieldIdentifier()).value = '' });
        document.getElementById(self._getIdentifier()).addEventListener('click', function (event)
        {
            event = event || window.event;
            var target = event.target || event.srcElement;
            if (m_helper.parent(target, self._getIdentifier())) { event.stopPropagation() }
        });
        ...
        var onInput = function ()
        {
            ...
            if (!text || text == '') {
                ...
                m_helper.each(array, function (e) { m_helper.show(e.parentElement.parentElement) })
            }
        }
    ...
    _onCheckBoxChange: function (checkbox, self, event) {
        if (!checkbox.dataset.multiselectElement) {
            ...
            if (typeof self._checkboxClickEvents[checkedState.id] === "function") { self._checkboxClickEvents[checkedState.id](checkbox, checkedState) }
        ...
        if (checkbox.checked) {
            self._itemCounter++; m_helper.select(this._item.options[item.index]);
            m_helper.setActive(checkbox.parentElement.parentElement);
            return { id: item.id, checked: !0 }
        }
        self._itemCounter--; m_helper.deselect(this._item.options[item.index]);
    ...
    _updateSelectAll: function (self) {
        ...
        if (self._itemCounter == self._getItems().length) { allChkBox.checked = !0 }
    ...
    _showList: function (context) { m_helper.setActive(document.getElementById(context._getItemListIdentifier())) }, _forceUpdate: function () {
    ...
    _getItemUniqueIdentifier: function () {
        var id = this._item.getAttribute('id'), name = this._item.getAttribute('name');
        if (!(id || name)) { throw "Multiselect: object does not contain any identifier (id or name)" }
        return id ? id : name
    },
    _getIdentifier: function () { return this._getItemUniqueIdentifier() + '_multiSelect' }, _getInputFieldIdentifier:
        function () { return this._getItemUniqueIdentifier() + '_input' }, _getItemListIdentifier:
        function () { return this._getItemUniqueIdentifier() + '_itemList' }, _getInputBadgeIdentifier:
        function () { return this._getItemUniqueIdentifier() + '_inputCount' }
}
window.multiselects = [];
if (typeof ($) != 'undefined') {
